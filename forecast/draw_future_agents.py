...
def draw_predictions(center_agent_annotation: Dict[str, Any],
                     center_agent_pixels: Tuple[float, float],
                     base_image: np.ndarray,
                     predictions,
                     resolution: float = 0.1) -> None:
    ...
    for path_idx in range(predictions.probabilities.shape[0]):
        for time_step in range(predictions.prediction.shape[1]):
            anno = dict()
            anno['translation'] = predictions.prediction[path_idx, time_step, :]
            anno['size'] = [0.5, 0.5]
            anno['rotation'] = [1, 0, 0, 0]
            box = get_track_box(anno, (agent_x, agent_y), center_agent_pixels, resolution)
...
class FutureAgentBoxesWithFadedHistory(AgentRepresentation):
    """
    ...
    def make_representation(self, instance_token: str, sample_token: str, sec_forward=-2, predictions=None) -> np.ndarray:
        ...
        if predictions is not None:
            draw_predictions(center_agent_annotation, central_track_pixels, base_image, predictions, resolution=self.resolution)
